...
async function setStatusIcon(state) {
  ...
  await chrome.action.setIcon({
    path: {
      16: `icons/${s}/icon16.png`,
      48: `icons/${s}/icon48.png`,
      128: `icons/${s}/icon128.png`,
    },
  });
...
async function restoreIcon() {
  const { capturedCurl, listenState } = await chrome.storage.local.get([
    "capturedCurl",
    "listenState",
  ]);
...
function tryComplete(requestId) {
  ...
  stopListening(true); // 捕获后停止，并保留绿色图标
}
...
async function stopListening(keepIcon = false) {
  ...
  if (debugTabId !== null) {
    try {
      await chrome.debugger.detach({ tabId: debugTabId });
    } catch (_) {}
    debugTabId = null;
  }
